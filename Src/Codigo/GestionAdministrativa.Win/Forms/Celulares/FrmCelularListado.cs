...
{
    public partial class FrmCelularListado : FormBaseListado
    {
        private readonly IMovilesNegocio _movilesNegocio;


        public FrmCelularListado(IFormFactory formFactory, IGestionAdministrativaUow uow, IMovilesNegocio movilesNegocio)
        {
                    ...
                    Task.Run(
                        () =>
                            //_movilesNegocio.Listado(SortColumn, SortDirection, numero, patente, activo, 1, 50,
                                                        _movilesNegocio.Listado(SortColumn, SortDirection, null, null, null, 1, 50,
        ...
        {
            using (var formCrear = FormFactory.Create<FrmCrearEditarCelular>(Guid.Empty, ActionFormMode.Create))
            {
        ...
        {
            using (var formCrear = FormFactory.Create<FrmCrearEditarCelular>(movilId, ActionFormMode.Edit))
            {
                var result = formCrear.ShowDialog();
                if (result == DialogResult.OK)
                {
